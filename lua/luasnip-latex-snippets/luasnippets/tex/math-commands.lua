...
M = {
  ...
  autosnippet({ trig = 'part', name = 'partial', dscr = 'Partial symbol' },
    ...
    \partial <>
      ...
      { i(0) }),
    { condition = tex.in_math, show_condition = tex.in_math }),
  autosnippet({ trig = 'sp', name = 'Scalar product', dscr = 'Scalar product (using angled braces)' },
    fmta([[
    \innerproduct{<>}{<>}<>
    ]],
      { i(1),i(2),i(0) }),
    { condition = tex.in_math, show_condition = tex.in_math }),
    autosnippet({ trig = 'sv', name = 'Vector subspace', dscr = 'Subspace generated by vectors' },
    fmta([[
    \generators{<>}<>
    ]],
      { i(1),i(0) }),
...
local greek_specs = {
  ...
  lam = { context = { name = "λ" }, command = [[\lambda]] },
  Lam = { context = { name = "Λ" }, command = [[\Lambda]] },
